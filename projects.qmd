...
# You Shall Not Pass — parallelisering og spilteori på en fælles server(Theoretical/Conceptual)

[You Shall Not Pass](Projects/parallelisering.qmd)

In this project, I look at resource allocation on a shared server through the lens of game theory, framing parallelization among multiple actors as a Nash equilibrium problem.
I ran simulations to see the calculation times with different actors on the same server, and niceness settings.
Note that the project itself is written in Danish, with Python for the simulations.
