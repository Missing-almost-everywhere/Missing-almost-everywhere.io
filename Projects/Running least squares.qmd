...
Look at $(X^TX)[j,g]=x_1^Tx_1$ So $\sum_{i=1}^{n}x_{j}[i]x_{g}[i]$
...
$(X_{new}X_{new}^T)[j,g]=(\sum_{i=1}^{n}x_{1}[j]x_{1}[g])+x_{new}[j]x_{new}[g]$
...
note here that $y_new$ act as a scale
