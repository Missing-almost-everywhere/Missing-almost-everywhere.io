...
The code is written in R, and I consider it what I call "one-time code." It’s not meant to be read by others, so there aren’t a lot of comments.
It’s not optimized either—I’ve simply chosen the fastest code I could think of or find to solve a problem.
...
# This should be made into tabs with shiny live.
...
```{r,eval=FALSE}
write.csv(df, "data_before_celan.csv", row.names = FALSE)
```
...
```{r,eval=FALSE}
write.csv(df, "data_after_celan.csv", row.names = FALSE)
```
...
data_after_celan <- read.csv("https://github.com/Missing-almost-everywhere/Missing-almost-everywhere.io/blob/main/Projects/house-prices-advanced-regression-techniqueshouse-prices-advanced-regression-techniques-data/data_before_celan.csv", stringsAsFactors = FALSE)
data_after_celan<-as.data.frame(data_after_celan)
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("variable", "Select Variable:", 
                  choices = names(data_after_celan))
    ),
    mainPanel(
      plotOutput("dynamicPlot")
    )
...
server <- function(input, output) {
  ...
  output$dynamicPlot <- renderPlot({
    ...
    if (is.numeric(data_after_celan[[col]])) {
      # For numeric variables, create a histogram
      n_unique <- length(unique(data_after_celan[[col]]))
      ggplot(data_after_celan, aes_string(x = col)) + 
        geom_histogram(bins = min(n_unique, 30), fill = 'skyblue', color = 'black', alpha = 0.7) + 
        labs(title = paste('Histogram of', col), x = col, y = 'Frequency') + 
        theme_minimal()
    } else {
...
## Looking at some no numeric varibels 
...
```{r,eval=FALSE}
write.csv(df, "data_final_for_model.csv", row.names = FALSE)
```
...
data_final <- read.csv("https://github.com/Missing-almost-everywhere/Missing-almost-everywhere.io/blob/main/Projects/house-prices-advanced-regression-techniqueshouse-prices-advanced-regression-techniques-data/data_final_for_model.csv", stringsAsFactors = FALSE)
data_final<-as.data.frame(data_final)

non_numeric_cols <- data_final %>%
  select_if(~!is.numeric(.)) %>%
  colnames()

for (col in non_numeric_cols) {
  p <- ggplot(data_final, aes_string(x = col, y = "log(SalePrice)")) +  
    geom_violin() +
    stat_summary(fun = "mean", geom = "point", color = "red", size = 3) +  # Tilføj mean som rødt punkt
    labs(title = paste("Violin plot for", col, "vs log(SalePrice)")) +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
server <- function(input, output) {
  ...
  output$dynamicPlot <- renderPlot({
    ...
    if (is.numeric(data_final[[col]])) {
      # For numeric variables, create a histogram
      n_unique <- length(unique(df[[col]]))
      ggplot(data_final, aes_string(x = col)) + 
        geom_histogram(bins = min(n_unique, 30), fill = 'skyblue', color = 'black', alpha = 0.7) + 
        labs(title = paste('Histogram of', col), x = col, y = 'Frequency') + 
        theme_minimal()
    } else {
...
So more heavy tale distribution,that a normal distribution. In many ways this model seam resanbull from a intuition point of view.  
...
## Some simple models for prediction
I wanted to measure the outsample predition streght of the above model, but i get to NA in the predition. Their is ways aoround this.One is to make the number making the number of groups the same as ther number of points, this coputasinal expsensiv, expsailly when i run the loop in R, this could be improved by writting my own C++ aplication Linear models is fitted with least squars estimatin wich has a closed form solution, making this somewhate reasonibell to implement. 
Second option is to make the groups in CV to make sure that when on is taken out the combinations is still being represnted ore the rank-deficient fits error do not acure.
Both of these methods could be intresting and fun but it not this project
To some extend one shoul also consinder if the data fundation can justifie a model if one can not supset the data to a very reasonbel size and stil get af fit. So i have made simpler model. 

Below here i have fitted to more still simple linear models, the preditiv stregnth seam to be somwhat reasonabel.
...
As observed, the predictive strength of the model is better than the baseline model, but not by a significant margin in terms of predictive performance.
...
Overall it seame like the best option the lasso time for predition.
So i did not combine the test and traning set when did the data cleaning, I did make function wich apllied the change i made to testset, but as can bee see from the two table belowe.
in the ms subclase their the value 150 wich was not pressent in the traning. I can remove since i summit a list to kaggle if i remove it will not alig with right value. so i am gona set it to 160
...
Looking at the data their is still some missing looking at the plot below.
...
I will input the data. i can't omit the rows.
here i will just use KNN since it works for cagorial vairbels
